...
# 2. FUNÇÃO DE MONITORAMENTO (Com detalhe da porta)
def check_port(ip_port):
    try:
        ...
        s.close()
        return result == 0, target_port # Retorna o status e a porta testada
    except:
        return False, 80
...
def montar_card(dados, titulo, icone, chave, cor_emoji):
    with st.container(border=True):
        # Agora ele mostra qual porta está pingando
        ...
        st.write(f"Status: **{status_txt}** (Porta: {porta_teste})") # Exibe a porta aqui
